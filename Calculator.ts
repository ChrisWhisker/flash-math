class Calculator {
  ...
  private shiftFactor = (f1: number, f2: number): number => {
    ...
    console.log(
      `Choice ${wrongAnswer} generated using Shifted Factor approach ${newEquation}`
    );
    return wrongAnswer;
  };
  ...
  private makeNewProduct = (): number => {
    ...
    console.log(
      `Choice ${wrongAnswer} generated using New Equation approach (${newFactor1} x ${newFactor2})`
    );
    return wrongAnswer;
  };
